...
def prepare_corpus(df, m1_name, m2_name, m1_dir, m2_dir, projects_flag):
...
if __name__ == '__main__':
    ...
    corpus, metadata = prepare_corpus(
        ...
        args.model1_dir, args.model2_dir, args.project
    )
    tfidf_df = prepare_tfidf(corpus, metadata, test_projs)
    counts_df = prepare_counts(corpus, metadata, test_projs)

    filename_tfdif = 'tfidf_projects.csv' if args.project else 'tfidf_proofs.csv'
    filename_counts = 'counts_projects.csv' if args.project else 'counts_proofs.csv'
